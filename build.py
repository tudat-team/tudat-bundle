...
def is_tudatpy_installed(build_directory: Path) -> bool:
    """Check if a build directory contains an installation manifest

    The function considers installation manifests from current (i.e. manifests/<name>.txt), or past (i.e. custom-manifest.txt) versions of the installation script.

    :param build_directory: Path to the build directory
    :return: True if installation manifest exists in chosen build directory, and False otherwise.
    """

    # Check for regular installation manifests
    manifest_dir: Path = build_directory / "manifests"
    if manifest_dir.is_dir():

        # If any txt files in directory, installation found
        if any(manifest_dir.glob("*.txt")):
            return True

    # Check for manifests from legacy installation script
    if (build_directory / "custom-manifest.txt").is_file():
        return True
...
class StubGenerator:
    ...
    def create_stubs_directory_structure(self) -> None:
    ...
    def generate_default_kernel_stubs(
        self, output_buffer: io.TextIOWrapper | None = None
    ) -> None:
    ...
    def generate_default_python_stubs(
        self, output_buffer: io.TextIOWrapper | None = None
    ) -> None:
    ...
    def fix_autogenerated_stubs(self) -> None:
    ...
    def generate_init_stubs(self) -> None:
    ...
    def generate_stubs(
        ...
        self.create_stubs_directory_structure()

        # Generate default stubs for tudatpy.kernel
        self.generate_default_kernel_stubs(output_buffer=output_buffer)

        # Generate default stubs for Python source code
        self.generate_default_python_stubs(output_buffer=output_buffer)

        # Fix autogenerated stubs
        self.fix_autogenerated_stubs()

        # Missing: Fix __init__.pyi stubs to expand star imports
        self.generate_init_stubs()
...
class Builder:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # Resolve path to build directory
        ...
        # Source directories
        self.tudat_bundle_dir = Path(__file__).parent

        # Source directory of tudatpy
        self.python_source_dir = self.tudat_bundle_dir / "tudatpy/src/tudatpy"
        # if not self.python_source_dir.exists():
        #     raise FileNotFoundError(
        #         f"Failed to generate stubs: "
        #         f"Source directory {self.python_source_dir} does not exist."
    ...
    def mock_environment(self) -> Generator[Environment, None, None]:
        """Mock conda environment in temporary directory

        Generates a temporary directory for the build, and a mock Conda environment to fool the stub generator, and sphinx into thinking that tudat and tudatpy are already installed.
        """
    ...
    @contextmanager
    def output_redirection_manager(
        self, redirect_output: bool
    ) -> Generator[io.TextIOWrapper | None, None, None]:
        """Utility to allow writing cmake output to file

        Provides a writeable buffer to a txt file in the build directory, or None if the user wants the cmake output in stdout. If logging to a file is requested, it ensures that the buffer is closed by the end of the build, even if it terminated with an error.

        :param redirect_output: Whether to redirect the output of subprocesses to a file in the build directory.
        """

        # If saving output to file requested, set up destination
        output_destination: Path | None = None
        if redirect_output:
            output_destination = self.build_dir / "build_output.txt"
            output_destination.parent.mkdir(parents=True, exist_ok=True)

        # Initialize output file buffer
        output_file_buffer: io.TextIOWrapper | None = None
        try:

            # Open buffer if output destination found
            if output_destination is not None:
                output_file_buffer = output_destination.open("w")

            yield output_file_buffer
    ...
    def run_cmake_setup_command(
        self,
        output_file_buffer: io.TextIOWrapper | None,
    ) -> None:
        """Run cmake command to set up build directory

        Executes `cmake -B <build-dir> -S <source-dir>` with additional arguments based on user configuration. The command creates or updates the build directory, as a preliminary step for the build process.

        :param output_to_file: Buffer object to log output and errors of cmake commands.
        :raises CalledProcessError: If the cmake command finishes with non-zero exit code. In this case, the build directory is removed to prevent conflicts with future builds.
        """

        # Check if past setup can be reused, return
        if not self.args.force_setup and self.build_dir.exists():
            return None

        # Initialize empty build directory if not present
        self.build_dir.mkdir(parents=True, exist_ok=True)

        # Run cmake setup command as subprocess
        setup_outcome = subprocess.run(
            [
                "cmake",
                ...
                f"{self.build_dir}",
                "-S",
                f"{self.tudat_bundle_dir}",
            ],
            stdout=output_file_buffer,
            stderr=output_file_buffer,
        )

        # Return with error if setup command failed
        if setup_outcome.returncode:

            # Remove build directory
            print("CMake setup failed: removing build directory")
            shutil.rmtree(self.build_dir)

            # Raise CalledProcess error
            setup_outcome.check_returncode()

        return None

    def run_cmake_build_command(
        self, output_file_buffer: io.TextIOWrapper | None
    ) -> None:
        """Run cmake command to build tudat and tudatpy

        Executes `cmake --build <build-dir>` with additional arguments based on user configuration. The command builds tudat and the C++ extensions of tudatpy.

        :param output_to_file: Buffer object to log output and errors of cmake commands.
        :raises CalledProcessError: If the cmake command finishes with non-zero exit code.
        """

        # Set up command based on command line input
        ...
        # Run build command as subprocess
        build_outcome = subprocess.run(
            build_command, stdout=output_file_buffer, stderr=output_file_buffer
        )
        build_outcome.check_returncode()

        return None

    def clean_build_directory(self) -> None:

        # Require removing previous installations before clean build
        if is_tudatpy_installed(self.build_dir):
            print(
                "WARNING\n"
    ...
    def build_libraries(self) -> None:
        ...
        if self.build_dir.exists() and self.args.clean_build:
            ...
            # If tudatpy is installed, exit with warning
            if is_tudatpy_installed(self.build_dir):
                print(
                    "WARNING\n"
                    "Installation manifests were found in the build "
        ...
        with self.output_redirection_manager() as output_buffer:

            # Set up build directory
            self.run_cmake_setup_command(output_buffer)

            # Build libraries
            self.run_cmake_build_command(output_buffer)
    ...
    def generate_api_docs(
        ...
        output_buffer: io.TextIOWrapper | None = None,
    ) -> None:
        """Generate API documentation for the library

        :param mock_env: Mock environment with tudatpy installed
        :param output_buffer: Buffer to log output and errors of the sphinx command.
        :raises CalledProcessError: If the sphinx command finishes with non-zero exit code.
        """
    ...
    def generate_stubs(
        ...
        stub_generator = StubGenerator(self.build_dir, mock_env)

        # Create directory for stubs in build
        if stub_generator.stubs_dir.exists():
            shutil.rmtree(stub_generator.stubs_dir)
        stub_generator.stubs_dir.mkdir(exist_ok=False, parents=False)

        # Create directory structure for stubs
        stub_generator.create_stubs_directory_structure()

        # Generate default stubs for tudatpy.kernel
        stub_generator.generate_default_kernel_stubs(output_buffer)

        # Generate default stubs for Python source code
        stub_generator.generate_default_python_stubs(output_buffer)

        # Fix autogenerated stubs
        stub_generator.fix_autogenerated_stubs()

        # Generate stubs for __init__.py files
        stub_generator.generate_init_stubs()
...
if __name__ == "__main__":
    ...
    with builder.output_redirection_manager(
        args.output_to_file
    ) as output_buffer:
        ...
        if not args.skip_build:
            ...
            # Set up build directory
            builder.run_cmake_setup_command(output_buffer)

            # Build tudat and tudatpy
            builder.run_cmake_build_command(output_buffer)

        # Initialize mock installation environment for stubs and API
        with builder.mock_environment() as mock_env:

            # Generate stubs
            if not args.skip_stubs:
                builder.generate_stubs(mock_env, output_buffer)

            # Generate API documentation
            if args.build_api_docs:
                builder.generate_api_docs(mock_env, output_buffer)
